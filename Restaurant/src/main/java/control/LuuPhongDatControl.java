 ...
 * @author thuan
 */
@WebServlet(name = "LuuPhongDatControl", urlPatterns = {"/luuphong"})
public class LuuPhongDatControl extends HttpServlet {
...
//        processRequest(request, response);
        ...
        try {
            ...
            PhongDatDAO dao = new PhongDatDAO();
            
            int hoadonid = dao.luuHoaDon(Integer.parseInt(khid));
            int phongdatid = dao.luuPhongDat(giovao, giora, 0, ngay, Integer.parseInt(phongid), hoadonid);
            
            
            for (int i = 0; i < nhanviencaids2.size(); i++) {
                dao.luuNVCPhongdat(nhanviencaids2.get(i),phongdatid);
            }
